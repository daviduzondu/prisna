...
import {AlertCircle, EyeIcon, MapPin, Phone, PlusCircleIcon, Star} from "lucide-react";
        ...
        <section className="w-[100%]">
                ...
                <div className="flex items-center">
                    <div className="flex-1 flex items-center">
                        ...
                        <div className="text-base lg:text-2xl font-bold ">
                            {user && user.uid === userDetails.data.uid && "Welcome back,"} {userDetails.data.name}
                        </div>
                    </div>
            ...
            <span className="flex items-center gap-1">
              <MapPin className="icon"/>
                {userDetails.data.location || ""}
                        ...
                        {user && user.uid === userDetails.data.uid && (
                            <Button
                                variant="destructive"
                                onClick={async () => {
                ...
                <AlertDialog open>
                    <AlertDialogContent>
                        ...
                        <Alert className="bg-red-500 text-white">
                            ...
                            <AlertDescription>
                                You have not completed your account set-up. Your account will not be visible to the
                                public until this is
                                done.
                            </AlertDescription>
                        </Alert>
            ...
            {userDetails.data && (
                <>
                    ...
                    <ProfileTabs user={user} userDetails={userDetails}/>
                </>
            )}
        </section>
    );
    ...
    return (
        <div className="flex lg:flex-row flex-col-reverse w-full gap-2 my-7 items-center lg:static">
            <div className="flex-1 w-full action-btns flex gap-3 sticky top-10">
                {user && user.uid === userDetails.data.uid && (
                    <AddListing showModal={true}>
                        <AlertDialogTrigger>
                            <Button asChild className="h-fit lg:h-full lg:w-fit w-full bg-black hover:bg-green-700 top-10 min-w-[200px]">
                                <div
                                    className="flex lg:flex-col items-center justify-center lg:w-fit w-[100%] hover:bg-blue-10 lg:p-4 cursor-pointer rounded-sm flex-1 lg:text-5xl text-2xl top-10  py-3">
                ...
                <Button asChild className="h-fit lg:w-fit w-full bg-green-600 hover:bg-green-700  top-10">
                    <Link href={`tel:${userDetails.data.phone}`}>
                        <div
                            className="flex lg:flex-col items-center justify-center lg:w-fit w-[100%] hover:bg-blue-10 lg:p-4 cursor-pointer rounded-sm flex-1 lg:text-5xl text-2xl top-10  py-3">
                            <Phone className="icon mr-2"/>
                            <span className="text-base lg:text-xl lg:mt-4">{userDetails.data.phone || "No Data"}</span>
                        </div>
                    </Link>
                </Button>
            </div>
