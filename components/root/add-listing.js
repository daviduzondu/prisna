...
export default function AddListing({ showModal }) {
  ...
  const [state, formAction] = useFormState(addNewListing, null);
    ...
    <>
      {isClient && (
        ...
        <Button variant="outline">Show Dialog</Button>
          ...
          <AlertDialogContent>
            <form onSubmit={() => router.push("?add=true")} className="flex flex-col gap-4" action={addNewListing}>
              <AlertDialogHeader>
                <AlertDialogTitle>Add a new listing</AlertDialogTitle>
              </AlertDialogHeader>
              <div className="flex flex-col gap-3">
                <Label htmlFor="event-name">Venue name</Label>
                <Input placeholder="Enter a catchy name for this center" name="event-name" type="text" required />
              </div>
              <div className="flex flex-col gap-3">
                <Label htmlFor="event-location">Location</Label>
                <Input placeholder="Where is this center located?" name="event-location" type="text" required />
              </div>{" "}
              <div className="flex flex-row gap-1 items-center ">
                ...
                <EventTypeList className="min-w-full" value={value} setValue={setValue} />
              </div>{" "}
              <FileChooser />
              <AlertDialogFooter>
